...
function Pill({ children }) {
  return (
    <span style={{ padding:"4px 12px", background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:999, fontSize:11, color:"#7a6a5a" }}>
      {children}
    </span>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom:4, display:"flex", alignItems:"center", gap:8 }}>
        <span style={{ fontSize:9, letterSpacing:3, color:"#C9A84C", textTransform:"uppercase" }}>{pergunta.cat}</span>
        <span style={{ fontSize:9, color:"rgba(255,255,255,0.12)" }}>·</span>
        <span style={{ fontSize:9, letterSpacing:2, color:"#4a3a2a", textTransform:"uppercase" }}>{numero} de {total}</span>
      </div>
      <h2 style={{ fontSize:"clamp(16px,3.5vw,21px)", fontWeight:400, color:"#f0e8d8", lineHeight:1.45, marginBottom:6 }}>
        {pergunta.texto}
      </h2>
      <p style={{ fontSize:13, color:"#6a5a4a", lineHeight:1.6, marginBottom:22, fontStyle:"italic" }}>
        {pergunta.sub}
      </p>
          ...
          return (
            <button key={i}
              ...
              style={{
                ...
                background: isSel ? `${c}1E` : hov === i ? "rgba(255,255,255,0.055)" : "rgba(255,255,255,0.025)",
                border: isSel ? `1.5px solid ${c}66` : hov === i ? "1px solid rgba(255,255,255,0.16)" : "1px solid rgba(255,255,255,0.07)",
                borderRadius:13, cursor: selecionada !== null ? "default" : "pointer", textAlign:"left",
              ...
              <span style={{
                ...
                border: isSel ? `2px solid ${c}` : "1px solid rgba(255,255,255,0.14)",
                background: isSel ? `${c}20` : "transparent",
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize: isSel ? 12 : 11, fontWeight:700, color: isSel ? c : "#7a6a5a",
                marginTop:1, transition:"all 0.2s",
              }}>
                {isSel ? "✓" : opt.l}
              </span>
              <span style={{ fontSize:14, color: isSel ? "#f5ede0" : "#c4b8a4", lineHeight:1.6, fontFamily:"Georgia,serif" }}>
                {opt.texto}
              </span>
            </button>
          );
...
function PastoralCard({ pastoral, rank, aiData, maxPts }) {
  ...
  return (
    <div style={{
      background: pastoral.bg, border:`1px solid ${pastoral.color}${isFirst?"60":"40"}`,
      borderRadius: isFirst ? 18 : 14, padding: isFirst ? "24px 20px" : "16px 16px",
      marginBottom: isFirst ? 16 : 10, position:"relative", overflow:"hidden",
      boxShadow: isFirst ? `0 0 40px ${pastoral.color}18` : "none",
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom: isFirst ? 12 : 0 }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          {isFirst && (
            <div style={{ fontSize:9, letterSpacing:3, color:pastoral.color, textTransform:"uppercase", marginBottom:4 }}>
              Sua Principal Pastoral
            </div>
          )}
          <div style={{ fontSize: isFirst ? 17 : 14, color:"#f0e8d8", fontWeight:400, lineHeight:1.3 }}>
            {pastoral.nome}
          </div>
          {!isFirst && (
      ...
      {isFirst && (
        <>
          <p style={{ fontSize:13, color:"#a09888", lineHeight:1.7, margin:"0 0 12px" }}>{pastoral.desc}</p>
          {aiData && (
            <>
              <div style={{ borderTop:"1px solid rgba(255,255,255,0.07)", paddingTop:12, marginBottom:12 }}>
                <p style={{ fontSize:13, color:"#b8aa98", lineHeight:1.75, margin:"0 0 10px" }}>{aiData.chamado}</p>
                <p style={{ fontSize:12, color:pastoral.color, fontStyle:"italic", margin:"0 0 12px" }}>{aiData.versiculo}</p>
                {aiData.passos && (
                  <div style={{ background:"rgba(255,255,255,0.03)", padding:"10px 14px", borderRadius:8, borderLeft:`2px solid ${pastoral.color}50`, marginBottom:4 }}>
                    <span style={{ fontSize:11, fontWeight:700, color:pastoral.color, display:"block", marginBottom:4, letterSpacing:1, textTransform:"uppercase" }}>Próximos Passos</span>
                    <p style={{ fontSize:12, color:"#8a7a68", lineHeight:1.7, margin:0 }}>{aiData.passos}</p>
                  </div>
                )}
              </div>
            </>
...
export default function App() {
  ...
  return (
    <>
      <style>{css}</style>
      <div style={{ minHeight:"100vh", background:"#0a0806", fontFamily:"Georgia,'Times New Roman',serif", color:"#e0d6c8", display:"flex", flexDirection:"column" }}>
        {/* Ambiente */}
        ...
        {fase === "quiz" && (
          <>
            <div ref={topoRef} style={{ padding:"11px 18px", borderBottom:"1px solid rgba(255,255,255,0.05)", display:"flex", alignItems:"center", justifyContent:"space-between", background:"rgba(0,0,0,.35)", backdropFilter:"blur(8px)", position:"sticky", top:0, zIndex:10 }}>
              <span style={{ fontSize:11, color:"#7a6a5a" }}>Teste Vocacional · N. Sra. de Fátima</span>
        ...
        {fase === "resultado" && recomendados.length > 0 && (
          <>
            <div style={{ padding:"11px 18px", borderBottom:"1px solid rgba(255,255,255,0.05)", background:"rgba(0,0,0,.35)", backdropFilter:"blur(8px)", position:"sticky", top:0, zIndex:10, textAlign:"center" }}>
              <span style={{ fontSize:11, color:"#7a6a5a" }}>Resultado Vocacional · {nome}</span>
            </div>
            <div style={{ flex:1, overflowY:"auto", padding:"28px 18px 60px", maxWidth:640, margin:"0 auto", width:"100%" }}>
